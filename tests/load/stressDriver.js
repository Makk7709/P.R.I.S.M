...
class StressTestDriver {
  ...
  generateEvent(priority) {
    const eventTypes = {
      ...
      NORMAL: [
        'prism:data:processed',
        'prism:monitoring:heartbeat',
        'prism:user:interaction'
      ]
    };
  ...
  updateLatencyStats(latency) {
    ...
    if (this.testResults.latencies.length > 0) {
      perf.averageLatency = this.testResults.latencies.reduce((a, b) => a + b, 0) / this.testResults.latencies.length;
    }
  ...
  async runStressTest() {
    ...
    console.log(`📊 Target: ${this.config.totalEvents} events over ${this.config.testDurationSeconds} seconds`);
    console.log(`⚡ Rate: ${this.config.criticalEventsPerSecond} CRITICAL/s, ${this.config.highEventsPerSecond} HIGH/s, ${this.config.normalEventsPerSecond} NORMAL/s`);
  ...
  generateReport() {
    ...
    const consensusSuccessRate = this.testResults.consensusMetrics.totalRequests > 0 
      ? this.testResults.consensusMetrics.approved / this.testResults.consensusMetrics.totalRequests 
      : 1.0;
...
async function runStressTest() {
  ...
  try {
    ...
    console.log(`Consensus Success Rate: ${(report.qualityMetrics.consensusSuccessRate * 100).toFixed(2)}%`);
