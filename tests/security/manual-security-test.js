...
class MockTrustContext {
  ...
  requiresHumanApproval(type, level, _data) {
    ...
    console.log(
      `🔍 Checking approval requirement for ${type} (${level}): ${requiresApproval ? 'REQUIRED' : 'NOT REQUIRED'}`
    );
  ...
  approveDecision(token, supervisorId, signature, reason = '') {
    ...
    if (
      !this.config.allowedSupervisors.includes(supervisorId) &&
      !supervisorId.startsWith('test_supervisor_')
    ) {
  ...
  getSecurityMetrics() {
    const rate =
      this.securityMetrics.totalDecisions > 0
        ? this.securityMetrics.approvedDecisions / this.securityMetrics.totalDecisions
        : 0;
...
async function runSecurityTests() {
  ...
  if (
    ...
    metrics.approvedDecisions >= 1 &&
    metrics.humanApprovalRate > 0
  ) {
    console.log('✅ PASS: Security metrics correctly tracked');
    console.log(
      `📊 Metrics: ${metrics.totalDecisions} total, ${metrics.approvedDecisions} approved, ${(metrics.humanApprovalRate * 100).toFixed(1)}% approval rate`
    );
    testsPassed++;
  } else {
...
runSecurityTests()
  .then((results) => {
    ...
    fs.writeFileSync('reports/security_verification.json', JSON.stringify(report, null, 2));
