...
export class TaskTypeProcessor {
  ...
  async process(userInput, taskType = 'general', options = {}) {
    ...
    try {
      ...
      const collaborationDecision = this.interDomainOrchestrator.shouldUseMultiDomain(
        userInput,
        taskType
      );
        ...
        persona = this.personaActivator.activate(taskType, {
          ...
          context: {
            ...
            projectContext: activeProject
              ? this.projectManager.getProjectContext(activeProject.id)
              : null,
          },
        });
      }
      ...
      console.log(
        `[TaskTypeProcessor] MemoryContext enrichedContext length:`,
  ...
  _needsRealTimeResearch(taskType, input) {
    ...
    return (
      researchRequiredTypes.includes(taskType) ||
