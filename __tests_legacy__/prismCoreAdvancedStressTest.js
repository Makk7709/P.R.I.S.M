...
describe('PRISM Core Advanced Stress Test', () => {
  ...
  test('should handle 50000 events with advanced monitoring', async () => {
    ...
    const results = {
      summary: {
        ...
        failures: 0,
        overloads: alerts.length,
        cpuUsage: systemMetrics[systemMetrics.length - 1]?.cpu.usage || 0,
        memoryUsage: systemMetrics[systemMetrics.length - 1]?.memory.used || 0
      },
      details: {
    ...
    // Save results
    await writeFile(
      'prismCoreAdvancedStressResults.json',
      JSON.stringify(results, null, 2)
    );
    
    // Verify results
    expect(results.summary.totalEvents).toBe(50000);
    expect(results.summary.failures).toBe(0);
    expect(results.summary.averageLatencyMs).toBeLessThan(500);
    expect(results.details.eventLatencies.length).toBe(50000);
    expect(eventsProcessed).toBe(50000);
    expect(completedEvents.length).toBe(50000);
    
    // Verify performance monitor was called correctly
    expect(performanceMonitor.handleEventStart).toHaveBeenCalledTimes(50000);
    expect(performanceMonitor.handleEventEnd).toHaveBeenCalledTimes(50000);
