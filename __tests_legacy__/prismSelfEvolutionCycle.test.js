...
jest.mock('../persistence/prismStateStore.js');
// jest.mock('../backend/database.js', () => ({
//   saveMemorySnapshot: jest.fn().mockResolvedValue(undefined),
//   fetchLatestSnapshots: jest.fn(),
// }));

// Import des modules mockés
import { launchSelfEvolutionCycle } from '../evolution/launchSelfEvolutionCycle.js';
...
describe('PRISM Self Evolution Cycle', () => {
  ...
  test('should complete full evolution cycle successfully', async () => {
    ...
    await launchSelfEvolutionCycle();
  ...
  it('devrait exécuter un cycle complet d\'analyse et de suggestion', async () => {
    // ...
    
    // Vérifier que les snapshots ont été demandés et que la mémoire a été sauvegardée
    // expect(fetchLatestSnapshots).toHaveBeenCalledWith(5);
    // expect(saveMemorySnapshot).toHaveBeenCalledWith(expect.any(Object));
  });

  it('should launch the full evolution cycle successfully', async () => {
    // ... Mocks pour une exécution réussie ...
    const mockPerplexityResponse = {
      // ...
    };
    jest.spyOn(perplexity, 'getPerplexityResponse').mockResolvedValue(mockPerplexityResponse);

    await launchSelfEvolutionCycle();

    expect(perplexity.getPerplexityResponse).toHaveBeenCalled();
  });

  it('should handle validation failure gracefully', async () => {
    // ... Mocks pour une validation échouée ...
    jest.spyOn(perplexity, 'getPerplexityResponse').mockResolvedValue({ valid: false });
    
    await launchSelfEvolutionCycle();
  });

  it('should handle quality check failure gracefully', async () => {
    // ... Mocks pour un contrôle qualité échoué ...
    const mockPerplexityResponse = {
      // ...
    };
    jest.spyOn(perplexity, 'getPerplexityResponse').mockResolvedValue(mockPerplexityResponse);

    await launchSelfEvolutionCycle({ expectedKeywords: ['non_existent_keyword'] });
  });
}); 