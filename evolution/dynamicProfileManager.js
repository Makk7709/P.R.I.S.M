...
class DynamicProfileManager extends EventEmitter {
  ...
  updateProfile(userId, taskResult) {
    ...
    taskPrefs.count++;
    taskPrefs.successRate = (taskPrefs.successRate * (taskPrefs.count - 1) + (taskResult.success ? 1 : 0)) / taskPrefs.count;
    taskPrefs.averageResponseTime = (taskPrefs.averageResponseTime * (taskPrefs.count - 1) + taskResult.responseTime) / taskPrefs.count;
  ...
  updatePerformanceMetrics(profile, taskResult) {
    ...
    // Update success rate
    metrics.successRate = (metrics.successRate * (metrics.successRate > 0 ? 1 : 0) + (taskResult.success ? 1 : 0)) / 2;
    
    // Update average response time
    metrics.averageResponseTime = (metrics.averageResponseTime * (metrics.averageResponseTime > 0 ? 1 : 0) + taskResult.responseTime) / 2;
    
    // Calculate and update quality scores
    const qualityScores = this.calculateQualityScores(taskResult);
    metrics.qualityScore = (metrics.qualityScore * (metrics.qualityScore > 0 ? 1 : 0) + qualityScores.quality) / 2;
    metrics.accuracyScore = (metrics.accuracyScore * (metrics.accuracyScore > 0 ? 1 : 0) + qualityScores.accuracy) / 2;
    metrics.relevanceScore = (metrics.relevanceScore * (metrics.relevanceScore > 0 ? 1 : 0) + qualityScores.relevance) / 2;
    metrics.consistencyScore = (metrics.consistencyScore * (metrics.consistencyScore > 0 ? 1 : 0) + qualityScores.consistency) / 2;
  }
