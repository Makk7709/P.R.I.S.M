...
// Middleware pour parsing JSON avec limite de taille
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    // Log de la requête
    console.log(`[PRISM API] Nouvelle requête chat: taskType=${taskType || 'general'}, model=${model || 'auto-select'}`);
    ...
    // Log de succès
    console.log(`[PRISM API] Requête traitée avec succès en ${processingTime}ms`);
    
  } catch (error) {
    const processingTime = Date.now() - startTime;
    
    // Log d'erreur
    console.error(`[PRISM API] Erreur lors du traitement:`, error);
    ...
    <body>
        <div class="container">
            ...
            <div class="nav-grid">
                ...
                <a href="/investor" class="nav-card" style="border: 2px solid #FFD700; background: linear-gradient(135deg, rgba(255, 215, 0, 0.1), rgba(255, 165, 0, 0.1));">
                    <h3>💎 Dashboard Investisseurs</h3>
                    <p><strong>PREMIUM</strong> - Interface ultra-haute définition dédiée aux présentations investisseurs. Design Silicon Valley niveau IPO.</p>
                </a>
            </div>
...
// Route Investisseur Premium (Isolée)
app.get('/investor', (req, res) => {
  res.redirect('/demo/investor-dashboard/');
});

app.get('/investor-premium', (req, res) => {
  res.redirect('/demo/investor-dashboard/');
});
